...
class TestDocumentComments:
    def test_create_list_update_and_delete(self, employee_client, history_document):
        ...
        listed = employee_client.get(f"/api/v1/documents/{history_document.id}/comments/")
        assert listed.json()["data"]["count"] == 1

        updated = employee_client.patch(
            f"/api/v1/comments/{comment_id}/", {"text": "Исправлено"}, format="json"
        )
    ...
    def test_other_user_cannot_edit_comment(
        ...
        response = auth_client(other).patch(
            f"/api/v1/comments/{comment.id}/", {"text": "Взлом"}, format="json"
        )
    ...
    @pytest.mark.parametrize(
        "comment_type",
        [
            DocumentCommentType.APPROVAL,
            DocumentCommentType.RETURN_REASON,
            DocumentCommentType.SYSTEM,
        ],
    )
    def test_non_general_comment_is_immutable(
        ...
        patch = employee_client.patch(
            f"/api/v1/comments/{comment.id}/", {"text": "Изменение"}, format="json"
        )
        delete = employee_client.delete(f"/api/v1/comments/{comment.id}/")
...
class TestDocumentHistory:
    ...
    def test_submit_approve_and_comments_are_recorded(
        ...
        employee_client.post(
            f"/api/v1/documents/{history_document.id}/submit/",
            {"approvers": [str(manager.id)]},
            format="json",
        )
        manager_client.post(
            f"/api/v1/documents/{history_document.id}/approve/",
            {"comment": "Согласовано"},
            format="json",
        )
    ...
    def test_return_and_resubmit_are_recorded(
        ...
        employee_client.post(
            f"/api/v1/documents/{history_document.id}/submit/", payload, format="json"
        )
        manager_client.post(
            f"/api/v1/documents/{history_document.id}/return/",
            {"comment": "Исправить"},
            format="json",
        )
        employee_client.post(
    ...
    def test_file_upload_and_delete_are_recorded(
        ...
        uploaded = employee_client.post(
            f"/api/v1/documents/{history_document.id}/files/",
            {
        ...
        with django_capture_on_commit_callbacks(execute=True):
            employee_client.delete(f"/api/v1/document-files/{file_id}/")
        assert list(history_document.history.values_list("action", flat=True)) == [
    ...
    def test_registration_completion_archive_and_restore_are_recorded(
        ...
        history_document.save(update_fields=["status"])
        admin_client.post(f"/api/v1/documents/{history_document.id}/register/")
        employee_client.post(f"/api/v1/documents/{history_document.id}/complete/")
        admin_client.post(f"/api/v1/documents/{history_document.id}/archive/")
        admin_client.post(f"/api/v1/documents/{history_document.id}/restore/")
        assert list(history_document.history.values_list("action", flat=True)) == [
    ...
    def test_history_endpoint_is_read_only(self, employee_client, history_document):
        response = employee_client.get(f"/api/v1/documents/{history_document.id}/history/")
        assert response.status_code == 200
        assert (
            employee_client.post(
                f"/api/v1/documents/{history_document.id}/history/", {}, format="json"
            ).status_code
            == 405
        )
