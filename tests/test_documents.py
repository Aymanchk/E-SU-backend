...
class TestDocumentCrud:
    ...
    def test_inactive_category_rejected(self, employee_client, child_department, document_category):
    ...
    def test_author_edits_draft(self, employee_client, document):
        response = employee_client.patch(
            f"/api/v1/documents/{document.id}/", {"title": "Изменено"}, format="json"
        )
    ...
    def test_submitted_document_cannot_be_edited(self, employee_client, document, manager):
        ...
        response = employee_client.patch(
            f"/api/v1/documents/{document.id}/", {"title": "Нельзя"}, format="json"
        )
        assert response.status_code == 400

    def test_draft_delete_is_soft(self, employee_client, document):
        response = employee_client.delete(f"/api/v1/documents/{document.id}/")
...
class TestDocumentVisibility:
    def test_employee_does_not_see_another_users_document(
        ...
        response = auth_client(other).get(f"/api/v1/documents/{document.id}/")
        assert response.status_code == 404

    def test_responsible_sees_document(self, auth_client, user_factory, child_department, document):
        ...
        response = auth_client(responsible).get(f"/api/v1/documents/{document.id}/")
        assert response.status_code == 200

    def test_manager_sees_department_document(self, manager_client, document):
        assert manager_client.get(f"/api/v1/documents/{document.id}/").status_code == 200
    ...
    def test_manager_does_not_see_other_department(
        ...
        assert manager_client.get(f"/api/v1/documents/{other_document.id}/").status_code == 404

    def test_admin_sees_everything(self, admin_client, document):
        assert admin_client.get(f"/api/v1/documents/{document.id}/").status_code == 200
...
class TestDocumentActions:
    ...
    def test_archive_and_restore(self, admin_client, document):
        ...
        archived = admin_client.post(f"/api/v1/documents/{document.id}/archive/")
        assert archived.status_code == 200
        document.refresh_from_db()
        assert document.status == DocumentStatus.ARCHIVED

        restored = admin_client.post(f"/api/v1/documents/{document.id}/restore/")
    ...
    def test_employee_cannot_archive(self, employee_client, document):
        ...
        assert employee_client.post(f"/api/v1/documents/{document.id}/archive/").status_code == 403


class TestDocumentListsAndFilters:
    def test_my_documents(self, employee_client, document):
        response = employee_client.get("/api/v1/documents/my/")
        assert response.status_code == 200
        assert response.json()["data"]["count"] == 1

    def test_status_and_priority_filters(self, employee_client, document):
        included = employee_client.get("/api/v1/documents/?status=draft&priority=high")
        excluded = employee_client.get("/api/v1/documents/?status=approved")
        assert included.json()["data"]["count"] == 1
        assert excluded.json()["data"]["count"] == 0

    def test_search_title(self, employee_client, document):
        response = employee_client.get("/api/v1/documents/?search=оборудования")
        assert response.json()["data"]["count"] == 1

    def test_special_lists(self, employee_client, document):
        document.status = DocumentStatus.OVERDUE
        document.save(update_fields=["status"])
        assert employee_client.get("/api/v1/documents/overdue/").json()["data"]["count"] == 1
        assert employee_client.get("/api/v1/documents/returned/").json()["data"]["count"] == 0
